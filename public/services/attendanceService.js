angular.module('app').service('attendanceService', function($http, config) {
  ...
  this.getDays = cohortId =>
    $http
      .get(`${this.url}?admin_token=${config.admin_token}&cohortId=${cohortId}`, {
        headers: {
  ...
  this.getDataFromDays = data => {
    ...
    for (const datum of data) {
      if (datum.day) {
        promises.push(
          $http.get(`${this.url}/${datum.cohortId}/${datum.day}?admin_token=${config.admin_token}`, {
            headers: {
              'Access-Control-Allow-Origin': '*',
            },
          })
  ...
  this.getAttendanceFromData = daysData => {
    ...
    for (const attend of daysData) {
      ...
      for (const student of attend.data.attendances) {
