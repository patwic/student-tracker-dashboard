angular.module('app').directive('surveyBarChart', function() {
  return {
    ...
    controller: function($scope, surveyService) {
      ...
      const num = 9;
      const width = document.getElementById('barDiv').offsetWidth - margin.right - margin.left;
      const height = document.getElementById('barDiv').offsetHeight - margin.top - margin.bottom - 80;
      ...
      const tip = d3
        ...
        .html(function(d) {
          ...
          return (
            `Average Rating: <span style='color:#21AAE1; line-height: 1.5;'> ${d[survey]}</span>` +
            `<br>` +
            `Responded: <span style='color:#21AAE1; line-height: 1.5;'> ${d[count]}</span>`
          );
        })
        .style('font-size', '11px');
      const svg = d3
        .select('#surveyBarChart')
        .append('svg')
      ...
      changeBar = (surveyType, surveyData) => {
        ...
        const newFilteredData = surveyData;
        console.log(newFilteredData);
        const newData = averages(newFilteredData);
        ...
        const tip = d3
          ...
          .offset([-15, 0])
          .html(function(d) {
            const count = `${newSurvey}count`;
            return (
              `Average Rating: <span style='color:#21AAE1; line-height: 1.5;'> ${ 
              d[newSurvey] 
              }</span>` +
              `<br>` +
              `Responded: <span style='color:#21AAE1; line-height: 1.5;'> ${ 
              d[count] 
              }</span>`
            );
          })
