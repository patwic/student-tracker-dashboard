angular.module('app').directive('helpLineChart', function() {
  return {
    restrict: 'AE',
    template: "<div id='helpLineChart'></div>",
    scope: {
      helpTimeData: '=',
    },
    controller: function($scope) {
      ...
      const data = makeDataObject($scope.helpTimeData);
      ...
      const width = document.getElementById('helpQDiv').offsetWidth - margin.right - margin.left;
      const height = document.getElementById('helpQDiv').offsetHeight - margin.top - margin.bottom - 50;
      ...
      let tickText = width < 275 ? d3.timeFormat('%I') : d3.timeFormat('%I:%M');
      ...
      x.domain([new Date($scope.helpTimeData[0][0]), new Date(endTime)]);
      ...
      const overlayWidth = width * (($scope.helpTimeData.length - 1) / 100) - 1;
      ...
      const updateCohortLineChart = someData => {
        let newData = makeDataObject(someData);
        ...
        let ya = d3.select('#helpLineChart').selectAll('.y.axis');
        ...
        let gradient = d3.select('#helpLineChart').selectAll('.area');
      ...
      $scope.$watch('helpTimeData', function(newValue, oldValue) {
        updateCohortLineChart($scope.helpTimeData);
      });
    },
  };
});
