angular.module('app').directive('waitLineChart', function() {
  return {
    restrict: 'AE',
    template: "<div id='waitLineChart'></div>",
    scope: {
      waitTimeData: '=',
    },
    controller: function($scope) {
      ...
      const data = makeDataObject($scope.waitTimeData);
      ...
      const width = document.getElementById('waitQDiv').offsetWidth - margin.right - margin.left;
      const height = document.getElementById('waitQDiv').offsetHeight - margin.top - margin.bottom - 50;
      ...
      const tickText = width < 275 ? d3.timeFormat('%I') : d3.timeFormat('%I:%M');
      ...
      x.domain([new Date($scope.waitTimeData[0][0]), new Date(endTime)]);
      ...
      const overlayWidth = width * (($scope.waitTimeData.length - 1) / 100) - 1;
      ...
      const updateCohortLineChart = someData => {
        ...
        const ya = d3.select('#waitLineChart').selectAll('.y.axis');
        ...
        const gradient = d3.select('#waitLineChart').selectAll('.area');
      ...
      $scope.$watch('waitTimeData', function(newValue, oldValue) {
        updateCohortLineChart($scope.waitTimeData);
      });
    },
  };
});
