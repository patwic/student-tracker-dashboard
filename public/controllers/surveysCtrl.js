angular.module('app').controller('surveysCtrl', function($scope, surveyService) {
  ...
  const approvedInstructorTopics = [
    'Javascript',
    'Git',
    'HTML/CSS',
    'CSS',
  ...
  surveyService.getTopics().then(res => {
    ...
    for (let i = 0; i < topics.length; i++) {
      for (let j = 0; j < approvedInstructorTopics.length; j++) {
        if (topics[i].name === approvedTopics[j]) {
          $scope.instructorTopics.push(topics[i]);
        }
      }
      for (let x = 0; x < approvedTopics.length; x++) {
  ...
  surveyService.getWeeklySurveyData().then(res => {
    ...
    const ios = $scope.allData.filter(e => e.program === 'ios');
    const webdev = $scope.allData.filter(e => e.program === 'webdev');
    const qa = $scope.allData.filter(e => e.program === 'qa');
    const ux = $scope.allData.filter(e => e.program === 'ux');
    $scope.allPrograms = {
      ios,
      webdev,
      qa,
  ...
  getAllSurveyData = (program, column) => {
    ...
    if ($scope.selectedProgram === 'all') {
      ...
      surveyData = $scope.allPrograms[$scope.selectedProgram].filter(e => e.program === $scope.selectedProgram);
    }
  ...
  makeSurveyLineChart = arr => {
    ...
    let surveyData = [];
    surveyData = $scope.allPrograms[$scope.selectedLineProgram].filter(e => e.program === $scope.selectedLineProgram);
    filteredData = surveyData;

    const averages = dataArr => {
      const arr = [];
      const obj = {};
      let max = 0;
      let min = 7;
      ...
      const ctx = document.getElementById('lineWeeklyChart');
      if ($scope.lineWeeklyChart) {
        $scope.lineWeeklyChart.destroy();
      }
  ...
  makeDataObject = arr => {
    ...
    const averages = dataArr => {
      ...
      const pastDate = priorDate.toDateString();
    ...
    const ctx = document.getElementById('surveyTopicLineChart');
  ...
  getInitialModuleList = () => {
    $scope.filteredModules = $scope.modules.filter(e => e.topicId === '56fb1628c63976af2f88b31c');
  };
  ...
  $scope.changeSelectedInstructorTopic = () => {
    ...
    $scope.filteredModules = $scope.modules.filter(e => e.topicId === $scope.instructorTopic);
  };
  ...
  $scope.changeSelectedModule = () => {
    if (!event.target.value) {
      ...
      const moduleData = $scope.instructorData.filter(e => e.module.name == event.target.value);
  ...
  makeInstructorObject = arr => {
    ...
    const averages = dataArr => {
      ...
      $scope.instructorBar;
      const ctx = document.getElementById('instructorBar').getContext('2d');
      if ($scope.instructorBar) {
        $scope.instructorBar.destroy();
      }
